...
* upload the bank data to hdsf (/spark/bank.csv)

```
   docker cp bank.csv CONTAINERID:/
   #login to the namenode container
   hadoop fs -mkdir /spark
   hadoop fs -put bank.csv /spark/
```

* access to zeppelin.(IP:NODEPORT)
...
```
   // upload the test data to /spark/bank.csv

val bankText = sc.textFile("hdfs://NAME-NODE-SERVICE:8020/spark/bank.csv")
